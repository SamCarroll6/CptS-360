...
{
   struct stat buf1; 
   if(stat(f1, &buf1))        //if f1 does not exist ==> exit.
   {
         printf("File f1 doesnt exists\n");
         return 0;
   }               
                  //f1 exists: reject if f1 is not REG or LNK file
   struct stat buf2;
   if(stat(f2, &buf2))   //reject if f2 exists and is not REG or LNK file
   {
         printf("File f2 doesnt exists\n");
         return 0;
   }
   printf("Both files exits\n");
//    3. if (f1 is REG){
